...
```{r echo=FALSE}
request_full <- "https://api.census.gov/data/2018/pep/population?get=GEONAME,POP&for=state:02&key=YOUR_KEY_GOES_HERE"
...
`fromJSON()` returns a matrix, not a tibble. We'll need to do some extra work to get the data into a tidy tibble. 
...
httr contains functions for extracting information out of response objects. `http_error()` returns `TRUE` if there was an error with your request, and `FALSE` otherwise.

```{r}
http_error(response)
```

We can also determine the type of data with `http_type()`.
...
To extract out the actual data, we can use `content()`.
...
For our Census Bureau case (and many other situations), `fromJSON()` is an easier solution. However, it's useful to know about the httr package, especially if you're working with an API in a script or package. `GET()` and `http_error()` allow you to programmatically handle errors, while `fromJSON()` will fail if something goes wrong. 
