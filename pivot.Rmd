...
`eagle_nests` contains data on the number of [bald eagle nesting sites](https://www.fws.gov/migratorybirds/pdf/management/EagleRuleRevisions-StatusReport.pdf) across multiple regions and years.
...
__Step 2:__ Identify the columns to pivot.
...
Let's see an example of a tibble that actually does need `pivot_wider()` to be tidy. `acs` contains data from the 2012 and 2017 [American Community Surveys](https://www.census.gov/programs-surveys/acs), obtained through the [tidycensus](https://walkerke.github.io/tidycensus/) package. 
