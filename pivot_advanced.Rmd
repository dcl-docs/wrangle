...
`pivot_longer()` and `pivot_wider()` are very flexible, and can easily tidy a wide variety of non-tidy datasets. The previous chapter only covered the basics. In this chapter, we'll explore this flexibility by introducing some of the pivot functions' optional functionality.

## Longer

### Types

You might have noticed that `pivot_longer()` always creates the `names_to` column as a character variable. Sometimes, you'll want this column to be a different type. For example, when we `pivot_longer()` `eagle_nests`, `year` becomes a character column.
...
`names_types` takes a named list. We'll show you an example and then dive into the details. Here's how we would create `year` as an integer column:
...
`integer()` is a _prototype_. Prototypes are vectors with a type, but no elements. Here, we use a prototype vector to define the desired type of `year`.
...
`pivot_longer()` also has a `values_ptypes` argument that controls the `values_to` column. You specify `values_ptypes` in the same way as `names_ptypes`. For example, say we wanted to change `num_nests` from its default type (double) to an integer.
...
However, `score_vault` isn't really a value of `event`. It would be better for the values to be `"vault"` and `"floor"`. We can remove the `"score_"` prefix with `pivot_longer()'` `names_prefix` argument.
...
you can give `pivot_longer()`'s `names_to` argument a vector of names in order to create more than one column. 

`pivot_wider()` has a similar functionality. You can use `pivot_wider()` to generate column names from multiple variables. For example, take the following data:

```{r}
dimensions_1
```

We might want a column for each combination of `estimate` and `measure`. We can do this by supplying `pivot_wider()` with multiple `names_from` variables.
...
In this case, we'll give `values_from` a vector.
