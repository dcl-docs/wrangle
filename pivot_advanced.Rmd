...
acs_2 <- read_rds(here::here("data/acs/acs_2016_2.rds"))
acs_3 <- read_rds(here::here("data/acs/acs_2016_3.rds"))
acs_4 <- read_rds(here::here("data/acs/acs_2016_4.rds"))
```
...
Sometimes, the columns you want to pivot will contain extra information, either in their names or their values. For example, `gymnastics_1` contains data on the scores of three countries' women's Olympic gymnastic teams in 2016. 
...
`"([:alpha:]+)"` matches only letters, so will pull out just `"vault"` or `"floor"`. `"(\\d+)"` matches digits and will pull out `2012` or `2016`. 
...
In `acs_2`, the names of our desired columns were stored across two variables: `variable` and `measure`. You might also encounter values stored across multiple variables. 
