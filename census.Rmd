...
Restart R (_Ctrl/Cmd_ + _Shift_ + _F10_) for the changes to take effect. 
...
`load_variables()` returns a tibble of all variable codes from a given dataset, alongside brief descriptions. We'll load the variables from the 2017 5-year ACS as an example. 
...
For `get_acs()`, `year` indicates the _end-year_ for the ACS estimates. If you want ACS estimates from 2012-2017, set `year = 2017`. By default, `get_acs()` uses the 5-year estimates. You can use other estimates by specifying `survey`.
...
The intercensals contain data between previous decennial census. For example, the 2000-2010 intercensal contains yearly data for 2000 through 2020. If you want data from, for example, 2000 to 2018, you'll need to use both a vintage and an intercensal, which will involve two API queries. 
...
The links under the API name provide you with more information about the API. If you want population data, you'll probably want the _Population Estimates_ API, which provides yearly population estimates are various geographic levels.
...
* `for=count:*&in=state:30` will return data by county, just for Montana.
